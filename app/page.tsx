...
// Gallery dynamic import (Placeholder)
...
export default function Home() {
  ...
  // Handle Initial Load & Check Local Storage
  useEffect(() => {
    setMounted(true);
    const storedTheme = localStorage.getItem("theme");
    const systemPrefersDark = window.matchMedia("(prefers-color-scheme: dark)").matches;

    // Apply cached theme, or fallback to system preference
  ...
  // Handle Dark Mode Toggle & Save to Cache
  ...
  return (
    <div className="w-full bg-[#F4F4F4] dark:bg-[#0B1120] transition-colors duration-300">
      
      {/* --- NAVIGATION BAR --- */}
      <nav className="fixed top-0 left-0 w-full h-16 z-50 bg-white/50 dark:bg-[#0B1120]/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 transition-colors duration-300">
      ...
      <main className="w-full flex flex-col items-center">
        ...
        {/* Projects Section - Removed internal paddings and width constraints for full-bleed background */}
